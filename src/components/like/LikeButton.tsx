...
export function LikeButton({ isDisabled, handleClick, isActive, isLoadingLike, hasUser }: Props) {
  return (
    <Button data-testid="like-button" disabled={isDisabled} size="icon" variant="ghost" onClick={handleClick} className={cn(isActive && 'hover:text-destructive transition-all', isLoadingLike && 'animate-pulse rounded-md bg-muted')}>
