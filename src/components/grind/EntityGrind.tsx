      ...
      {allEntityVisits.length === 0
        ...
        : (
          <div>
            ...
            <AnimationWrapper className="border border-muted sm:w-80">
              {allEntityVisits.map(card => (
                <div key={card.id} className="border-b border-muted py-10 flex items-center gap-1">
                  <Button onClick={() => handleEntityRemoved(card.id)} variant="ghost" className="hover:text-destructive" size="icon">
            ...
            <Button title="delete all" className="w-full" onClick={handleEntitiesRemoved} variant="outline" size="icon">
